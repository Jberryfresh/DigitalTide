...
const getArticlesSchema = Joi.object({
  query: Joi.object({
    ...
    status: Joi.string().valid('draft', 'pending_review', 'approved', 'published', 'archived').optional(),
    category: Joi.string().uuid().optional(),
    tag: Joi.string().uuid().optional(),
    search: Joi.string().optional(),
    sortBy: Joi.string().valid('published_at', 'created_at', 'view_count', 'quality_score').default('published_at'),
...
const createArticleSchema = Joi.object({
  body: Joi.object({
    ...
    slug: Joi.string().min(1).max(500).pattern(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).required(),
...
const updateArticleSchema = Joi.object({
  ...
  body: Joi.object({
    ...
    slug: Joi.string().min(1).max(500).pattern(/^[a-z0-9]+(?:-[a-z0-9]+)*$/).optional(),
...
const articleIdSchema = Joi.object({
  params: Joi.object({
    id: Joi.alternatives().try(
      Joi.string().uuid(),
      Joi.string().pattern(/^[a-z0-9]+(?:-[a-z0-9]+)*$/)
    ).required(),
  }),
});
...
router.get('/', optionalAuth, validate(getArticlesSchema), articlesController.getArticles);

// GET /api/v1/articles/:id - Get single article (public with optional auth)
router.get('/:id', optionalAuth, validate(articleIdSchema), articlesController.getArticle);

// POST /api/v1/articles - Create article (requires authentication)
router.post('/', authenticate, validate(createArticleSchema), articlesController.createArticle);

// PUT /api/v1/articles/:id - Update article (requires authentication)
router.put('/:id', authenticate, validate(updateArticleSchema), articlesController.updateArticle);

// DELETE /api/v1/articles/:id - Delete article (requires authentication)
router.delete('/:id', authenticate, validate(articleIdSchema), articlesController.deleteArticle);
